...
def choose(audits: list[dict]) -> dict:
    ...
    for item in audits:
        ...
        if reasons:
            rejected.append({"workId": item.get("workId"), "language": item.get("language", ""), "title": item.get("queryTitle", ""),
                              "domain": item.get("domain", ""), "detailUrl": item.get("detailUrl"), "status": item.get("status", ""),
                              "rejectionReasons": item.get("rejectionReasons", []), "selectionReasons": reasons})
        else:
