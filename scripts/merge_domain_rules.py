...
p=argparse.ArgumentParser();p.add_argument('--input-dir',type=Path,required=True);p.add_argument('--ledger',type=Path);p.add_argument('--output',type=Path,required=True);a=p.parse_args();rules_by_domain={};rejected=[]
ledger_domains=set()
if a.ledger and a.ledger.exists():
 ledger=json.loads(a.ledger.read_text(encoding='utf-8-sig'));ledger_domains={x['domain'] for x in ledger.get('domains',[]) if x.get('verifiedWorkCount',0) > 0}
old={}
if a.output.exists():
...
def precise_reader(audit):
 selectors=audit.get('readerImageSelectors',[])
 for selector in sorted(selectors,key=len):
  match=re.search(r'(img|source)\.([A-Za-z_][\w-]*)$',selector)
  if not match:continue
  tag,css_class=match.groups();klass=re.escape(css_class)
  return rf'<{tag}(?=[^>]*class=["\'][^"\']*\b{klass}\b[^"\']*["\'])[^>]+(?:data-original|data-src|data-lazy-src|data-url|src|srcset)=["\']([^"\']+)["\'][^>]*>',[1]
 return reader,[1,2]
def precise_chapter(audit):
 selectors=audit.get('detailChapterSelectors',[])
 for selector in sorted(selectors,key=len):
  match=re.search(r'a\.([A-Za-z_][\w-]*)$',selector)
  if not match:continue
  klass=re.escape(match.group(1))
  return rf'<a(?=[^>]*class=["\'][^"\']*\b{klass}\b[^"\']*["\'])[^>]+href=["\']([^"\']+)["\'][^>]*>([\s\S]{{0,280}}?)<\/a>'
 return chapter
for path in sorted(a.input_dir.glob('*.json')):
 d=json.loads(path.read_text(encoding='utf-8-sig'))
 if d.get('status')!='verified':rejected.append(d);continue
 domain=d['domain']
 reader_regex,reader_groups=precise_reader(d);chapter_regex=precise_chapter(d);rules_by_domain[domain]={'id':d['id'],'name':domain+' 已验证公开源','description':f"由 {d['replayWorkCount']} 部作品首/中/末章回放验证；每域一条规则。",'homepage':'https://'+domain,'searchUrl':'','searchMethod':'url-only','searchItemRegex':r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>([\s\S]{1,240}?)<\/a>','searchTitleGroups':[2],'searchUrlGroups':[1],'searchCoverGroups':[],'searchResultIsChapter':False,'searchFilterByKeyword':True,'detailChapterRegex':chapter_regex,'detailChapterTitleGroups':[2],'detailChapterUrlGroups':[1],'detailChapterFilter':True,'readerImageRegex':reader_regex,'readerImageGroups':reader_groups,'userAgent':'Mozilla/5.0 (Linux; HarmonyOS; Mobile) AppleWebKit/537.36 Chrome/124 Mobile Safari/537.36 ComicReader','referer':'https://'+domain+'/','maxReaderPages':12,'license':'MIT','sourceType':'verified-public-domain-rule','compliance':{'publicOnly':True,'noLoginRequired':True,'noPaymentBypass':True,'noCaptchaBypass':True},'domainApplicabilityList':[domain],'audit':d}
