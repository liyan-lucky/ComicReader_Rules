...
**已清空历史，从零验证流水线中。** 当前 `generated/` 目录为空，待规则生成和目录生成 workflow 运行后填充。

### 域名列表状态

| 语种 | 域名数 | 备注 |
|------|--------|------|
| zh-Hans | 268 | 已清理：796→204→268（移除592个非漫画站，发现新增64个） |
| zh-Hant | 666 | 待清理 |
| en | 578 | 待清理 |
...
- `config/domains/`：搜索域名配置（zh-Hans/zh-Hant/en 按语种分文件，已清理非漫画站）。
- `config/queries/`：域名发现搜索词（按语种分文件）。
- `config/blocked_domains.json`：屏蔽域名模式（discover_domains + generate_rules 两套，覆盖10大非漫画类别）。
- `config/manga_indicator_keywords.json`：漫画指示关键词（按语种，域名合理性验证用）。
- `config/aggregator_sites.json`：聚合站URL列表（按语种）。
...
- `scripts/discover_domains.py`：域名发现脚本（含Phase 3合理性验证）。
