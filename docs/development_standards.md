...
```text
main     稳定分支。App 正式默认读取。只有明确许可后才合并。
develop  开发分支。所有新增脚本、规则、目录、生成流程先进入这里。
```

默认要求：

- 新功能、新脚本、新规则、新 workflow 先提交到 `develop`。
- `main` 不做自动生成、不做自动同步、不做实验。
- 合并 `main` 前必须先确认 `develop` 的 Actions 运行结果。
- 生成类 workflow 默认只提交到 `develop`。
