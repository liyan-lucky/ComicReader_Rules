...
```text
main      正式稳定分支，App 默认读取。只有明确许可后才合并。
develop   开发测试分支，新功能、规则生成、目录生成、清理流程先提交到这里。
```

默认原则：

```text
所有新增脚本、规则、目录、生成流程先进入 develop。
main 不做自动生成、不做自动同步、不做实验。
确认 develop 运行结果 OK 后，再按明确许可合并 main。
```
...
```text
https://raw.githubusercontent.com/liyan-lucky/ComicReader_Rules/develop/generated/index.json
```
