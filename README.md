...
1. **每日平台文字目录**：腾讯动漫、哔哩哔哩漫画、快看漫画、咚漫、漫画岛、有妖气、看漫画、菠萝包漫画、动漫之家、漫画台分别运行。第一步只保留平台、分类和公开书名，不传递详情链接、封面或章节数据。
2. **分类连接检索与验证**：16个分类分别读取第一步已经规范化、去重的文字目录，再运行“逐书找源 → 首/中/末章节验证 → 最佳来源选择”。每类拥有独立配置、搜索服务、缓存和检查点；公共缺陷由共享核心同步修复，分类特例只修改该类配置。
3. **域名规则分析**：按域名汇总已验证书籍，推导一域一规则，并对该域名全部作品样本回放。
4. **发布**：只有具备封面、详情链接、真实章节、三章阅读证据和已验证域名规则的作品才进入 App 目录。

当前阅读质量策略为 `readability-v5`：正文审计排除缩略图条和页面装饰，域名分析生成的精确正文选择器必须原样进入 App 规则，避免章节只显示封面、推荐图或相邻章节缩略图。

## 独立工作流

- `01-collect-platforms.yml`：10个平台并行粗采集并按分类合并。
- `02-refine-categories.yml`：16条分类完整分级流水线并行执行，完成后统一建立域名账本并提交。
